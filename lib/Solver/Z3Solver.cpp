...
// Output time takesn to solve queries, useful for tuning delegation times.
...
llvm::cl::opt<unsigned>
    CSATimeout("csa-timeout", llvm::cl::init(70),
                     llvm::cl::desc("CSA timeout (in milliseconds) - set 0 to disable (default=70)"),
                     llvm::cl::cat(klee::SolvingCat));
...
Z3GlobalSolverDelegate::Z3GlobalSolverDelegate()
...
bool Z3GlobalSolverDelegate::internalRunSolver(
...
SolverImpl::SolverRunStatus Z3GlobalSolverDelegate::handleSolverResponse(
...
bool Z3GlobalSolverDelegate::validateZ3Model(::Z3_solver &theSolver, ::Z3_model &theModel) {
  bool success = true;
...
SolverImpl::SolverRunStatus Z3GlobalSolverDelegate::getOperationStatusCode() {
...
private:
  ...
  std::unique_ptr<Z3GlobalSolverDelegate> globalSolver;
...
public:
  ...
  void setCoreSolverTimeout(time::Span timeout) {
    globalSolver->setCoreSolverTimeout(timeout);
  }
...
Z3SolverImpl::Z3SolverImpl()
    : builder(new Z3Builder(
          ...
          /*z3LogInteractionFileArg=*/Z3LogInteractionFile.size() > 0
              ? Z3LogInteractionFile.c_str()
              : NULL)),
      runStatusCode(SOLVER_RUN_STATUS_FAILURE), globalSolver(std::make_unique<Z3GlobalSolverDelegate>()) {
  assert(builder && "unable to create Z3Builder");
  solverParameters = Z3_mk_params(builder->ctx);
  Z3_params_inc_ref(builder->ctx, solverParameters);
  timeoutParamStrSymbol = Z3_mk_string_symbol(builder->ctx, "timeout");

  // Set solver timeout, corresponds to CSA strategy.
  Z3_params_set_uint(builder->ctx, solverParameters, timeoutParamStrSymbol,
                     CSATimeout);

  // Set timeout of global solver.
  setCoreSolverTimeout(timeout);
...
bool Z3SolverImpl::internalRunSolver(
  ...
  if (runStatusCode == SolverImpl::SOLVER_RUN_STATUS_TIMEOUT) {
    ...
    runStatusCode = globalSolver->getOperationStatusCode();
    return ret;
  }
